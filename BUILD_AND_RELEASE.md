...
3.  Change the `Publisher` attribute to match the `$publisherName` you used in the PowerShell script (e.g., `"CN=YourCompanyName"`).
    ...
    <!-- After -->
    <Identity Name="ExpansionPlanning" Publisher="CN=PowerGEM" Version="1.0.0.0" />
    ```
...
2.  Run the following commands:
    ...
    $msixOutputFile = "ExpansionPlanning-Installer-v$($packageVersion).msix"
...
$appName = "Expansion Planning" # Define app name for easy reuse
...
try {
    ...
    Write-Host "1. Checking for existing versions of $appName..." -ForegroundColor Yellow
    $removedCount = 0

    # First, try to remove all-users packages (requires admin)
    $allUsersPackages = Get-AppxPackage -Name "*$appName*" -AllUsers -ErrorAction SilentlyContinue
    ...
    Write-Host "4. Installing $appName..." -ForegroundColor Yellow
    ...
    if ($msixPath) {
        ...
        Write-Host "   Success: $appName installed.`n"
    } else {
        throw "Main application package (.msix) not found."
    }

    Write-Host "--- Installation Complete! ---" -ForegroundColor Green
    Write-Host "You can now find and launch '$appName' from your Start Menu."

    # Step 5: Launch the Application
    Write-Host "5. Attempting to launch $appName..." -ForegroundColor Yellow
    $newlyInstalledPackage = Get-AppxPackage -Name "*$appName*" | Select-Object -First 1
    if ($newlyInstalledPackage) {
        $packageFamilyName = $newlyInstalledPackage.PackageFamilyName
        $appId = "$packageFamilyName!App"
        try {
            Start-Process "explorer.exe" "shell:AppsFolder\$appId" -ErrorAction Stop
            Write-Host "   Success: $appName launched.`n"
        } catch {
            Write-Host "   Warning: Failed to launch $appName automatically. You can launch it from the Start Menu." -ForegroundColor Yellow
        }
    } else {
        Write-Host "   Warning: Could not find installed $appName to launch automatically." -ForegroundColor Yellow
    }
...
The final output is a single zip file (e.g., `ExpansionPlanning-Release-v1.0.0.zip`) that you can send to your users.
